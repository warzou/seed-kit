...
def redact_runtime_config(config: dict[str, str]) -> dict[str, object]:
    return {
        ...
        "last_recovery_time": config.get("last_recovery_time", ""),
        "last_recovery_trigger": config.get("last_recovery_trigger", ""),
...
def write_runtime_config(config: dict[str, str]) -> None:
    ...
    lines = [
        ...
        f"last_recovery_time={config.get('last_recovery_time', '')}",
        f"last_recovery_trigger={config.get('last_recovery_trigger', '')}",
...
def update_runtime_config(payload: dict) -> tuple[dict, int]:
    ...
    previous_debug_passive = config.get("runtime_recovery_debug_passive", "true")
    ...
    if "runtime_recovery_critical_link_loss_seconds" in payload:
        ...
        if not critical.isdigit() or int(critical) < 1 or int(critical) > 3600:
            return {"status": "failure", "error": "runtime-recovery-critical-link-loss-invalid"}, 400
        config["runtime_recovery_critical_link_loss_seconds"] = str(int(critical))
    ...
    if config.get("runtime_recovery_debug_passive", "true") != previous_debug_passive:
        append_action_log(
            FAILSAFE_MODE_LOG,
...
def parse_key_value_text(line: str) -> dict[str, str]:
    values: dict[str, str] = {}
...
def local_timestamp_label(value: str) -> str:
    ...
    return local_dt.strftime("%Y-%m-%d %H:%M:%S %Z").strip()
...
def runtime_watchdog_status() -> dict[str, object]:
    ...
    event_values = parse_key_value_text(last_event)
    state_timestamp = state.get("timestamp", "")
    last_event_timestamp = event_values.get("timestamp", "")
    return {
        ...
        "last_event_local": local_timestamp_label(last_event_timestamp),
        "timestamp": state_timestamp,
        "timestamp_local": local_timestamp_label(state_timestamp),
